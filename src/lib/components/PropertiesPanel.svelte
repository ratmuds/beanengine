<script lang="ts">
    ...
    function isNode3D(obj: ObjectProperties): boolean {
        return obj.type === 'Node3D' || obj.type === 'Part' || obj.type === 'Light';
    }
...
<div class="h-full bg-gray-900/60 backdrop-blur-sm border-l border-gray-700/30 flex flex-col">
    <!-- Header -->
    <div class="px-3 py-2 border-b border-gray-700/30 bg-gray-800/20">
        <h2 class="text-gray-200 font-medium text-sm">Properties</h2>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
        {#if selectedObject}
            ...
            <div class="px-3 py-2 border-b border-gray-700/20">
                <Input
                    ...
                    class="bg-transparent border-0 text-gray-200 text-sm font-medium p-0 h-auto focus:ring-0 focus:border-0"
                    on:input={(e) => handlePropertyChange('name', e.target.value)}
                />
                ...
                <div class="border-b border-gray-700/20">
                    ...
                    {#if expandedSections.transform}
                        <div class="px-3 pb-2 space-y-2">
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Position</label>
                                <Vector3Input
                                    value={selectedObject.position || { x: 0, y: 0, z: 0 }}
                                    on:change={(e) => handlePropertyChange('position', e.detail.value)}
                                />
                            </div>
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Rotation</label>
                                <Vector3Input
                                    value={selectedObject.rotation || { x: 0, y: 0, z: 0 }}
                                    step={1}
                                    precision={1}
                                    on:change={(e) => handlePropertyChange('rotation', e.detail.value)}
                                />
                            </div>
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Scale</label>
                                <Vector3Input
                                    value={selectedObject.scale || { x: 1, y: 1, z: 1 }}
                                    step={0.1}
                                    on:change={(e) => handlePropertyChange('scale', e.detail.value)}
                                />
                            </div>
                        </div>
                    {/if}
            ...
            {#if isPart(selectedObject)}
                <div class="border-b border-gray-700/20">
                    ...
                    {#if expandedSections.part}
                        <div class="px-3 pb-2 space-y-2">
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Material</label>
                                <select
                                    ...
                                    on:change={(e) => handlePropertyChange('material', e.target.value)}
                                >
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div class="flex items-center justify-between">
                                    ...
                                    <Switch
                                        size="sm"
                                        checked={selectedObject.castShadows !== false}
                                        on:change={(e) => handlePropertyChange('castShadows', e.detail)}
                                    />
                                </div>
                                <div class="flex items-center justify-between">
                                    <label class="text-xs text-gray-400">Receive</label>
                                    <Switch
                                        size="sm"
                                        checked={selectedObject.receiveShadows !== false}
                                        on:change={(e) => handlePropertyChange('receiveShadows', e.detail)}
                                    />
                                </div>
                            </div>
                            <div class="flex items-center justify-between">
                                ...
                                <Switch
                                    ...
                                    on:change={(e) => handlePropertyChange('visible', e.detail)}
                                />
                            </div>
                        </div>
                    {/if}
            ...
            {#if isLight(selectedObject)}
                <div class="border-b border-gray-700/20">
                    ...
                    {#if expandedSections.light}
                        <div class="px-3 pb-2 space-y-2">
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Type</label>
                                <select
                                    value={selectedObject.lightType || "directional"}
                                    ...
                                    on:change={(e) => handlePropertyChange('lightType', e.target.value)}
                                >
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="flex gap-1.5">
                                    <input
                                        ...
                                        on:input={(e) => handlePropertyChange('color', e.target.value)}
                                    />
                                    <Input
                                        type="text"
                                        value={selectedObject.color || "#ffffff"}
                            ...
                            <div class="space-y-1">
                                <label class="text-xs text-gray-400 block">Intensity</label>
                                <Input
                                    type="number"
                                    ...
                                    on:input={(e) => handlePropertyChange('intensity', parseFloat(e.target.value) || 0)}
                                />
                            </div>
                        </div>
                    {/if}
            ...
            <div class="border-b border-gray-700/20">
                <div class="px-3 py-2">
                    <div class="flex items-center gap-2 text-sm font-medium text-gray-200 mb-2">
                    ...
                    <ClassSelector
                        ...
                        on:removeClass={(e) => dispatch('removeClass', e.detail)}
                    />
                </div>
            </div>
        ...
        {:else}
            ...
            <div class="flex flex-col items-center justify-center h-64 text-center px-4">
                ...
                <p class="text-gray-500 text-sm">Select an object to view properties</p>
            </div>
        {/if}
    </div>
</div>