<script lang="ts">
    ...
    import { BVector2 } from "$lib/types";

    let {
        value = $bindable(new BVector2(0, 0)),
        label = "",
        precision = 3,
        step = 0.1,
        disabled = false,
    ...
    async function copyVector() {
        const vectorString = `${formatNumber(value.x)}, ${formatNumber(value.y)}`;
        try {
    ...
    async function pasteVector() {
        try {
            ...
            if (values.length === 2 && values.every((v) => !isNaN(v))) {
                const updatedValue = new BVector2(values[0], values[1]);
                value = updatedValue;
                dispatch("change", { value: updatedValue });
            }
        } catch (err) {
