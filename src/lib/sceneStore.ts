...
class SceneManager {
    ...
    createObject(
        ...
        switch (objectType.toLowerCase()) {
            case "part":
                ...
                break;
            case "mesh":
                newObject = new Types.BMesh(name, null, null);
                if (newObject instanceof Types.BMesh) {
                    newObject.position =
                        options?.position || new Types.BVector3(0, 0, 0);
                    newObject.scale = new Types.BVector3(1, 1, 1);
                    if (options?.assetId) {
            ...
            case "camera":
                ...
                if (newObject instanceof Types.BCamera) {
                    ...
                    // isActive handling removed (property not on BCamera definition snapshot)
                }
                break;
            case "light":
    ...
    getActiveCamera(): Types.BCamera | undefined {
        // Active camera concept not supported (isActive property missing); return first camera
        return this.getAllCameras()[0];
    }

    setActiveCamera(cameraId: string): void {
        // Placeholder: no-op since cameras lack isActive property in this version
        void cameraId;
    }
...
function createSceneStore() {
    ...
    return {
        ...
        createObject: (
            ...
            let createdObject: Types.BObject | null = null;
