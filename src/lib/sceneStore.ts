...
class SceneManager {
  ...
  private async initializePhysics(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        console.log("[SceneStore] Physics world initialized successfully");
      } catch (error) {
        console.error(
  ...
  createObject(
    ...
    if (parentId) {
      ...
      if (parentObject) {
        console.log(`Parent object with ID ${parentId} found:`, parentObject);
  ...
  updateObject(object: Types.BObject) {
    const index = this.scene.objects.findIndex((obj) => obj.id === object.id);
    if (index !== -1) {
  ...
  public clearScene(createDefaults: boolean = true): void {
    ...
    if (createDefaults) {
      ...
      const defaultUIStorage = new Types.BUIStorage("UI Storage", null, null);
      this.scene.addObject(defaultUIStorage);
    }

    console.log("[SceneStore] Scene cleared - all objects and variables reset");
  }
  ...
  private serializeObject(obj: Types.BObject): SerializedObject {
    ...
    const isArrayOfBObjects = (value: unknown): boolean =>
      Array.isArray(value) && value.every((v) => v instanceof Types.BObject);
  ...
  public deserialize(data: SerializedScene): void {
    ...
    for (const objData of data.objects) {
      ...
      for (const key of Object.keys(objData)) {
        ...
        if (key.endsWith("Id")) {
          ...
          if (target) {
            (obj as unknown as Record<string, unknown>)[prop] = target;
          } else {
            (obj as unknown as Record<string, unknown>)[prop] = null;
          }
        } else if (
          key.endsWith("Ids") &&
          ...
          const ids = (objData as Record<string, unknown>)[key] as string[];
    ...
    const hasUIStorage = data.objects.some((obj) => obj.type === "uistorage");
    if (!hasUIStorage) {
      const defaultUIStorage = new Types.BStorage("UIStorage", null, null);
      this.scene.addObject(defaultUIStorage);
      console.log(
  ...
  private createObjectFromData(data: SerializedObject): Types.BObject | null {
    ...
    for (const key of Object.keys(data)) {
      ...
      if (
        ...
        (obj as unknown as Record<string, unknown>)[key] = new Types.BVector3(
          v3.x,
          v3.y,
          v3.z
        );
...
function createSceneStore() {
  ...
  return {
    ...
    createObject: (
      ...
      let createdObject: Types.BObject;
      update((currentManager) => {
        createdObject = currentManager.createObject(
          objectType,
          parentId,
          options
        );
    ...
    deleteCustomEvent: (id: string) => {
      update((currentManager) => {
        currentManager.customEvents = currentManager.customEvents.filter(
          (e) => e.id !== id
        );
        return currentManager;
      });
